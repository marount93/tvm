...
__global__ void convf(char *in_data, float *data) {

    int x;
    float o;

    x = (unsigned char)in_data[threadIdx.x] - 127;
    data[threadIdx.x] = __fdividef((float)x, 128);
    printf("%f\n",data[threadIdx.x]);
}

__global__ void lenet(char *indata,
        float *data,
        float *conv1_weight,
        float *conv2_weight,
        float *fc2_weight,
    ...
    convf<<<dimGrid_convf,dimBlock_convf>>>(indata, data);
...
//    dim3 dimGrid_softmax2(1,1,1),  dimBlock_softmax2(64,1,1);
    ...
    cudaMalloc(&indata, sizeof(char)* 28 * 28);
    ...
    load_input_from_file("data4.dat", indata, 3136,0);
    ...
    lenet<<<1,1>>>(indata, data, conv1_weight, conv2_weight, fc2_weight, fc2_bias, oo, o0, o1, o2, o3, o4);
    cudaDeviceSynchronize();
    printf("%s\n", cudaGetErrorString(cudaGetLastError()));
